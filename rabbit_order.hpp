...
namespace rabbit_order
{
    namespace aux
    {
        ...
        union atomix
        {
            ...
            operator T() const
            {
                return a.load(boost::memory_order_acquire);
            }
            T fetch_add(T x)
            {
                return a.fetch_add(x, boost::memory_order_acq_rel);
            }
            T exchange(T x)
            {
                return a.exchange(x, boost::memory_order_acq_rel);
            }
        ...
        struct graph
        {
            ...
            vint n() const
            {
                return static_cast<vint>(es.size());
            }
        };
        ...
        OutputIt compact(InputIt it, const InputIt last, OutputIt result)
        {
            ...
            std::sort(it, last, [](auto & e0, auto & e1)
            {
        ...
        bool check_result(graph *const pg)
        {
            ...
            const auto istop = [&g](const vint v)
            {
                return is_toplevel(g, v);
            };
